...
Training finished at 30,000 steps; the final checkpoint on the pod is `run_001/29999/`
(~8.9 GB orbax checkpoint). Host/port/key below match `RUNPOD_SETUP §0` and the current pod.
...
### 5c. One-time client install (skip if already done for the effort TA VLA)
...
| No competing GPU load | `nvidia-smi`                                                                                                                                              | enough free VRAM for the ~8.9 GB checkpoint |
| Checkpoint present    | `du -sh checkpoints/pi0_trossen_transfer_base/run_001/29999`                                                                                              | ~8.9G                                       |
